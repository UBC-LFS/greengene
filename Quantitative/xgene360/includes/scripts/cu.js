...
xgene360_cu.getElementLeft = function( objIdOrElement )
{
  var objElement = xgene360_cu.Element( objIdOrElement );
  
  var iLeft = objElement.offsetLeft;
...
xgene360_cu.getElementTop = function( objIdOrElement )
{
  var objElement = xgene360_cu.Element( objIdOrElement );
  
  var iTop = objElement.offsetTop;
  var objParent = objElement.offsetParent;
...
xgene360_cu.hideShowDisplay = function( objIdOrElement )
{
  var objElement = xgene360_cu.Element( objIdOrElement );
...
xgene360_cu.setDisabled = function( objIdOrElement, bDisabled )
{
  var objElement = xgene360_cu.Element( objIdOrElement );
  
  objElement.disabled = bDisabled;
}
...
xgene360_cu.checkDefaultSubmitButton = function( e, objButtonIdOrElement )
{
  xgene360_cu.using( 'event' );
  ...
  {
    var objButton = xgene360_cu.Element( objButtonIdOrElement );
...
xgene360_cu.component.dateExtension = function()
{
  Date.prototype.validate = function( iYear, iMonth, iDay, iHour, iMinute, iSecond )
  {
    return ( this.getFullYear() == iYear && this.getMonth() == iMonth && this.getDate() == iDay &&
              this.getHours() == iHour && this.getMinutes() == iMinute && this.getSeconds() == iSecond );
  }
  ...
  Date.prototype.toSQLFormat = function()
  {
      ...
      var strHours = "0" + ( this.getHours() + 1 );
      strHours = strHours.substring( strHours.length - 2 );
      
      var strMinutes = "0" + this.getMinutes();
      strMinutes = strMinutes.substring( strMinutes.length - 2 );
      
      var strSeconds = "0" + this.getSeconds();
      strSeconds = strSeconds.substring( strSeconds.length - 2 );
      
      return this.getFullYear() + '-' + strMonth + '-' + strDay + ' ' + strHours + ':' + strMinutes + ':' + strSeconds;
  }
}
...
xgene360_cu.component.event = function()
{
  ...
  xgene360_Event = function( e )
  {
    ...
    {
      this.keyCode = xgene360_cu.IE ? objEvent.charCode : objEvent.keyCode;
    }
    */
...
xgene360_cu.component.sortabletable = function()
{
  ...
  xgene360_cu.sortabletable.sortTable = function( objLink )
  {
    ...
    var strItem = xgene360_cu.getInnerText( objTable.rows[1].cells[xgene360_cu.sortabletable.sortingColumn] );
    ...
    if ( strItem.match( /^[\d\.]+$/ ) )
    {
      objSortFunc = xgene360_cu.sortabletable.sortNumeric;
    }
  ...
  xgene360_cu.sortabletable.sortNumeric = function( objA, objB )
  {
    var fItem1 = parseFloat( xgene360_cu.getInnerText( objA.cells[xgene360_cu.sortabletable.sortingColumn] ) );
    var fItem2 = parseFloat( xgene360_cu.getInnerText( objB.cells[xgene360_cu.sortabletable.sortingColumn] ) );
...
xgene360_cu.component.form = function()
{
  ...
  xgene360_cu.form.addRequiredCheckBox = function( strId, strMessage )
  {
    var objItem = new Object();
    
    objItem.strId = strId;
    objItem.strMessage = strMessage;
    objItem.objFunc = xgene360_cu.form.validateRequiredCheckBox;
    
    xgene360_cu.form.objValidates.push( objItem );
  }
  
  xgene360_cu.form.validateRequiredCheckBox = function()
  {  
    var objInputElements = document.getElementsByTagName( 'INPUT' );
    ...
    {
      if ( objInputElements[i].type == 'checkbox' &&
            objInputElements[i].name == this.strId &&
  ...
  xgene360_cu.form.addRequiredSelection = function( objIdOrElement, strMessage )
  {
    var objItem = new Object();
    
    objItem.objElement = xgene360_cu.Element( objIdOrElement );
    objItem.strMessage = strMessage;
    objItem.objFunc = xgene360_cu.form.validateRequiredSelection;
  ...
  xgene360_cu.form.validateRequiredSelection = function()
  {
    if ( xgene360_cu.stringLength( this.objElement.options[this.objElement.selectedIndex].value ) == 0 )
    {
      alert( this.strMessage );
      this.objElement.focus();
