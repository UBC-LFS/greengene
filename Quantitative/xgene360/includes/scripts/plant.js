function xgene360_plant( strPlantId, fTraitAValue, fTraitBValue )
{
  ...
  this.setTraitClass = function( bTraitA, iIndex, iMaxIndex )
  {
    var iIndexSpan = Math.ceil( iMaxIndex / xgene360_plantshelf_img_manager.iLeafImageCount );
    
    var strIndex = "00" + ( Math.floor( iIndex / iIndexSpan ) + 1 );
  ...
  this.update = function()
  {
    if ( this.m_objElement != null )
    {
      if ( this.m_bTraitASelected || this.m_bTraitBSelected )
      {
        ...
        {
          strSrc = xgene360_plantshelf_img_manager.getImage( 'pc_a' );
        }
      ...
      if ( this.m_bSelectionChanged )
      {
        if ( this.m_bSelected )
        {
          this.m_objElement.style.backgroundColor = '#ff6600';
        }
...
xgene360_plantshelf_img_manager.iLeafImageCount = 11;
...
xgene360_plantshelf.strTraitAName = null;
xgene360_plantshelf.strTraitBName = null;
...
xgene360_plantshelf.render = function( strCanvas, objPlants )
{
  ...
  for ( var i = 0; i < objPlants.length; ++i )
  {
    ...
    strPlantDescription = 'Trait A: ' + objPlants[i].fTraitAValue.toFixed( 3 ) + '<br />' +
                          'Trait B: ' + objPlants[i].fTraitBValue.toFixed( 3 );


    // determine which region this plant belongs to 
    
    strHTML +=  '<div class="p" id="' + objPlants[i].strPlantId + '" onmouseover="xgene360_cu.tip.show( \'' + strPlantDescription + '\', \'#90ee90\' );" onmouseleave="xgene360_cu.tip.hide();" onmouseout="if ( !xgene360_cu.IE || xgene360_cu.OPERA ) { xgene360_cu.tip.hide(); }">' +
...
xgene360_plantshelf.notifySelectionChange = function( objPlant )
{
  if ( xgene360_plantshelf.objListOfSelectPlantsTable != null )
  {
    ...
    if ( objPlant.m_bSelected )
    {
      ...
      objTraitACell.appendChild( document.createTextNode( objPlant.fTraitAValue.toFixed( 3 ) ) );
      
      var objTraitBCell = objNewRow.insertCell( 2 );
      objTraitBCell.appendChild( document.createTextNode( objPlant.fTraitBValue.toFixed( 3 ) ) );
      
      var objRemoveCell = objNewRow.insertCell( 3 );
      var objRemoveButton = document.createElement( 'INPUT' );
      objRemoveButton.type = 'button';
      objRemoveButton.className = 'buttoninput';
      objRemoveButton.value = 'X';
      objRemoveButton.onclick = objPlant.onclickHandler.bind( objPlant );
      objRemoveCell.appendChild( objRemoveButton );
...
xgene360_plantshelf.updateSelectedPlantsMeanAndVariance = function()
{
  ...
  if ( objTable != null )
  {
    ...
    else
    {
      ...
      if ( xgene360_cu.stringLength( xgene360_plantshelf.strTraitAName ) == 0 )
      {
      ...
      {
        objTraitCell.appendChild( document.createTextNode( xgene360_plantshelf.strTraitAName ) );
      }
      
      var objMeanCell = objRow.insertCell( 1 );
      objMeanCell.appendChild( document.createTextNode( fMeanA.toFixed( 3 ) ) );
      ...
      {
        objVarianceCell.appendChild( document.createTextNode( Math.sqrt( fVarianceA ).toFixed( 3 ) ) );
      }
      ...
      {
        objTraitCell.appendChild( document.createTextNode( xgene360_plantshelf.strTraitBName ) );
      }
      
      objMeanCell = objRow.insertCell( 1 );
      objMeanCell.appendChild( document.createTextNode( fMeanB.toFixed( 3 ) ) );
      ...
      {
        objVarianceCell.appendChild( document.createTextNode( Math.sqrt( fVarianceB ).toFixed( 3 ) ) );
      }
    }
  } 
}
