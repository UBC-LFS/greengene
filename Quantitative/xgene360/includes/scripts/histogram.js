...
function xgene360_histogram( strStyle, strSelectedStyle )
{  
  ////////////////////////////////////////////////////////////
  //
  // member variables
  ...
  this.m_bUserFixedRange = false;
  
  this.m_iXCount  = 21; // the number of bars horizontally
  ...
  this.m_strSelectedStyle = strSelectedStyle;
  
  this.m_strTraitName = null;
  ...
  this.setRange = function( fLowerRange, fUpperRange )
  {
    this.m_bUserFixedRange = true;
    this.m_fMinX = fLowerRange;
    this.m_fMaxX = fUpperRange;
  }
  
  this.assignValues = function( objPlants, bTraitA )
  {
    if ( !this.m_bUserFixedRange )
    {
      this.m_fMinX  = 0;
      this.m_fMaxX  = 0;
    }
    ...
    for ( var i = 0; i < objPlants.length; ++i )
    {
      ...
      objPlants[i].setTraitClass( bTraitA, iIndex, this.m_iXCount );
  ...
  this.renderSimple = function( strCanvas, iCanvasWidth, iCanvasHeight, iAxisHeight )
  {
    ...
    objCanvas.style.backgroundColor = '#ececec';
    ...
    var iBarWidth = Math.floor( iCanvasWidth / this.m_iXCount ) - 1;
    var iBarMaxHeight = iCanvasHeight - iAxisHeight;
    
    // container related stuff
    var objContainer = document.createElement( 'DIV' );
    
    objContainer.id = this.m_strContainerId;
    objContainer.style.width = iCanvasWidth + 'px';
    objContainer.style.height = ( iCanvasHeight - iAxisHeight ) + 'px';
    objContainer.style.position = 'absolute';
    objContainer.style.left = '0px';
    objContainer.style.top = iAxisHeight + 'px';
    ...
    for ( var i = 0; i < this.m_objRange.length; ++i )
    {
      ...
      // which trait is this?
      objBar.className = this.m_strStyle;
    ...
    this.m_objInfoLabel.style.top = '0px';
    this.m_objInfoLabel.style.fontSize = '9px';
    
    // hack to calculate mean/sd
    for ( var i = 0; i < this.m_objSelected.length; ++i )
    {
      this.m_objSelected[i] = true;
    }
    
    this.updateInfoLabel();
    
    objCanvas.appendChild( this.m_objInfoLabel );
  ...
  this.render = function( strCanvas, iCanvasWidth, iCanvasHeight, iAxisWidth, iAxisHeight )
  {
    ...
    if ( this.m_strTraitName != null )
    {
      objLabel.appendChild( document.createTextNode( this.m_strTraitName ) );
    }
    ...
    for ( var i = 0; i < this.m_objRange.length; ++i )
    {
      ...
      var strBarDescription = 'Count: ' + this.m_objRange[i].length + '<br />' +
                              'Range: ' + ( this.m_fMinX + i * this.m_fBarRange ).toFixed( 3 ) + ' - ' + ( this.m_fMinX + i * this.m_fBarRange + this.m_fBarRange ).toFixed( 3 );
      ...
      eval( 'objBar.onmouseover = function() { xgene360_cu.tip.show( \'' + strBarDescription + '\', \'lightyellow\' ); };' );
    ...
    for ( var i = 0; i < this.m_iYCount; ++i )
    {
      ...
      objAxisLabel.appendChild( document.createTextNode( ( this.m_iMaxY -  ( this.m_iMaxY / this.m_iYCount ) * i ).toFixed( 3 ) ) );
    ...
    for ( var i = 0; i < this.m_iXCount; ++i )
    {
      if ( i % 3 == 0 )
      {
        ...
        objAxisLabel.style.width = ( iBarWidth * 3 ) + 'px';
        objAxisLabel.style.left = ( iLeft + iAxisWidth + this.m_iXAxisOffset ) + 'px';
        
        objAxisLabel.appendChild( document.createTextNode( ( this.m_fMinX + i * this.m_fBarRange ).toFixed( 3 ) ) );
      }
  ...
  this.assignTraitName = function( strTraitName )
  {
    this.m_strTraitName = strTraitName;
  }
  ...
  this.updateInfoLabel = function()
  {
    ...
    if ( isNaN( fVariance ) )
    {
      this.m_objInfoLabel.innerHTML = '&nbsp;Mean: ' + fMean.toFixed( 3 ) + '&nbsp;';
    }
    
    else
    {
      this.m_objInfoLabel.innerHTML = '&nbsp;Mean: ' + fMean.toFixed( 3 ) + '&nbsp;&nbsp;SD: ' + Math.sqrt( fVariance ).toFixed( 3 ) + '&nbsp;';
    }
...
xgene360_histogram_drag_select.onmove = function( objInfo )
{
  if ( xgene360_histogram_drag_select.m_objMask != null )
  {
    ...
    for ( var i = 0; i < xgene360_histogram_drag_select.m_objBars.length; ++i )
    {
      ...
      var objBar = xgene360_histogram_drag_select.m_objBars[i];
      
      xgene360_histogram_drag_select.m_objHistogram.m_objSelected[i] = xgene360_histogram_drag_select.m_objSelectedBars[i];
      
      if ( xgene360_histogram_drag_select.m_objSelectedBars[i] )
      {        
        objBar.className = xgene360_histogram_drag_select.m_objHistogram.m_strSelectedStyle;
      ...
      {
        objBar.className = xgene360_histogram_drag_select.m_objHistogram.m_strStyle;
